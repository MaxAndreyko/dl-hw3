...
class TextDataset(Dataset):
    ...
    def __init__(
        ...
        self.train_texts, self.val_texts = self.split_train_val(texts)
        self.texts = self.train_texts if train else self.val_texts
        self.indices = [
            self.text2ids(text) for text in self.texts
        ]  # Генерируем индексы для соответствующих текстов
    ...
    @staticmethod
    def split_train_val(self, data: Union[list, np.ndarray]) -> Tuple[list, list]:
        """Splits data to train and validation sets

        Args:
            data (Union[list, np.ndarray]): _description_

        Returns:
            Tuple[list, list]: Two sets splitted
        """
        random.seed(self.TRAIN_VAL_RANDOM_SEED)
        random.shuffle(data)
        split_idx = int(len(data) * self.VAL_RATIO)
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, int]:
        ...
        """
        indices = self.indices[item]  # Получаем индексы входного текста

        # Добавляем специальные токены и обрезаем до максимальной длины
        indices = [self.bos_id] + indices + [self.eos_id]
        indices = indices[: self.max_length]

        # Дополняем до максимальной длины с помощью служебных токенов
